...
def main():
    ...
    print(f"[INFO] Starting sampling with tune={args.tune}, draws={args.draws} at {time.ctime(start_time)}")
    ...
    sampler_stats = nuts.create_sample(
        ...
        tune=args.tune,
        chains=1,
        target_accept=0.65 + 0.1 *np.exp(-0.25 *dim),
        maxdepth=args.depth,
        max_energy_error=args.error,
    )

    end_time = time.time()
    elapsed = end_time - start_time
    print(f"[INFO] Sampling finished in {elapsed:.2f} seconds ({elapsed/60:.2f} min)")
    
    nuts.statistics.save_to_nc(args.mol.replace(".mol", ".nc"))

    output_log = args.mol.replace(".mol", ".log")

    with open(output_log, "w") as f:
        f.write("=== Sampling Configuration ===\n")
        f.write(f"Input molecule: {args.mol}\n")
        f.write(f"Sampling start time: {round(elapsed/60, 2)}\n")
        f.write(f"Random seed: 42\n")
        f.write("\n--- Sampling Parameters ---\n")
        f.write(f"tune (warmup steps): {args.tune}\n")
        f.write(f"draws (sampling steps): {args.draws}\n")
        f.write(f"max_depth: {args.depth}\n")
        f.write(f"max_energy_error: {args.error}\n")
        f.write("\n--- System Info ---\n")
        f.write(f"Python version: {platform.python_version()}\n")
        f.write(f"NumPy version: {np.__version__}\n")
        f.write(f"RDKit version: {rdkit.__version__}\n")
        f.write(f"NutPie version: {nutpie.__version__}\n")        
        f.write("\n=== End of log ===\n")
    ...
    for chain in range(phi_samples.shape[0]):
        ...
        output_sdf = args.mol.replace(".mol", f"_conformations.sdf")
